...
bool SendTxtMessage(unsigned char kind, char *text)
{
    ...
    all[1] = SYNC1;
    all[2] = kind;
    all[3] = (unsigned char)(length & 0xFF);
    all[4] = (unsigned char)(length >> 8);

    memcpy(all + 5, buf, length);

    crc = 0;
    UpdateCrc(&crc, all, 5 + length);

    all[5 + length]     = (unsigned char)(crc & 0xFF);
