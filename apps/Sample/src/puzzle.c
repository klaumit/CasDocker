...
static byte LevelSet(void)
{
	...
	/* data */
	strcpy( C@ t_tbl[0],"Level1");
	strcpy( C@ t_tbl[1],"Level2");
	strcpy( C@ t_tbl[2],"Level3");
...
static void DspFileOpenErr(void){
	...
	LibPutProStr( B@@ IB_PFONT1,30,35, B@ "PV aplication memory",110);
	LibPutProStr( B@@ IB_PFONT1,30,44, B@ "is full!",110);
	LibPutProStr( B@@ IB_PFONT1,30,53, B@ "See your User's Guide",110);
	LibPutProStr( B@@ IB_PFONT1,30,62, B@ "for more infomation.",110);
...
void PuzzleDisplay(void){
	...
	LibPutProStr( B@@ IB_PFONT1,104,2, B@ "New",30);				/* New */
	LibPutProStr( B@@ IB_PFONT1,74,2, B@ "Level",30);			/* Level */
	LibPutProStr( B@@ IB_PFONT1,108,15, B@ "High score",50);		/* High score */
...
/******************************************************************************
*	[Title]		ChipsDisplay
*	[Arguments]	int  x, y  --- DrawPosition(0~3)
*			int  n  --- ChipNumber(0~15 :0=Space)
*			bool f  --- ( TRUE:Display )
*	[ReturnVAL]	None
******************************************************************************/
static void DispCard( int x, int y, int n, bool f ) {
...
static bool JudgeEnd( void ) {
	...
	if(HighScoreCk()==TRUE){
		...
		LibPutFarData(71,0,8);
		LibPutProStr( B@@ IB_PFONT1,79,2, B@ "Clr",18);				/* Clr */
		LibPutProStr( B@@ IB_PFONT1,106,2, B@ "Set",18);				/* Set */
		LibPutFarData((LCD_X_DOT-128)/2,27,107);
		LibPutFarData(75,58,135);

		LibPutProStr( B@@ IB_PFONT3,33,33, B@ "Congratulations!",110);	/* Congratulations! */
		LibPutProStr( B@@ IB_PFONT3,26,46, B@ "Time",50);				/* Time */
		LibPutProStr( B@@ IB_PFONT3,26,59, B@ "Name",50);				/* Name */

		Num2( &t[0], puzzleinfo.timer/120);
		t[2]=':';
		Num2( &t[3], (puzzleinfo.timer%120)/2);
		t[5]='\0';
		LibPutProStr( B@@ IB_PFONT3,97,46,t,34);
		...
		LibPutProStr( B@@ IB_PFONT3,33,38, B@ "Congratulations!",110);			/* Congratulations! */
		LibPutProStr( B@@ IB_PFONT3,((LCD_X_DOT-128)/2)+10,56, B@ "Time",50);	/* Time */
...
static void HighScoreSave(byte *name){
	...
	if(gamelevel == 0){
		strcpy( C@ sc->lv1, C@ name);
		sc->lv1time = puzzleinfo.timer;
	}else if(gamelevel == 1){
		strcpy( C@ sc->lv2, C@ name);
		sc->lv2time = puzzleinfo.timer;
	}else{
		strcpy( C@ sc->lv3, C@ name);
...
static bool HighScoreCk(void){
	...
	if(time == FF@  || time > puzzleinfo.timer)
		return(TRUE);
...
static void HighScoreClr(void){
	...
	if(sc->lv1time == FF@  && sc->lv2time == FF@  && sc->lv3time == FF@ )
		return;
...
static void DispHSData(void){
	...
	LibPutProStr( B@@ IB_PFONT2,DSP_HS_X+48,DSP_HS_Y+4, B@ "Level1",50);			/* Level1 */
	LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+13, B@ "Time",30);			/* Time */
	LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+25, B@ "Name",30);			/* Name */

	LibPutProStr( B@@ IB_PFONT2,DSP_HS_X+48,DSP_HS_Y+39, B@ "Level2",50);		/* Level2 */
	LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+48, B@ "Time",30);			/* Time */
	LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+60, B@ "Name",30);			/* Name */

	LibPutProStr( B@@ IB_PFONT2,DSP_HS_X+48,DSP_HS_Y+74, B@ "Level3",50);		/* Level3 */
	LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+83, B@ "Time",30);			/* Time */
	LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+95, B@ "Name",30);			/* Name */

	LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+17,DSP_HS_Y+112, B@ "Clr",30);			/* Clr */
	LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+99,DSP_HS_Y+112, B@ "OK",30);			/* OK */

	if(sc->lv1time != FF@ ){
		TimeChg(t,sc->lv1time);
		LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+13,t,34);
		LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+25,sc->lv1,58);
	}

	if(sc->lv2time != FF@ ){
		TimeChg(t,sc->lv2time);
		LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+48,t,34);
		LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+60,sc->lv2,58);
	}

	if(sc->lv3time != FF@ ){
		TimeChg(t,sc->lv3time);
		LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+83,t,34);
		LibPutProStr( B@@ IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+95,sc->lv3,58);
	}
	LibPutDisp();
}
